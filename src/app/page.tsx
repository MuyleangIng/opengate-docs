...
// Only custom animations & helpers that Tailwind can't generate
const CSS = `
...
function ArchDiagram() {
  ...
  const edges = EDGES.map((e,i) => ({ ...e, id:`e${i}`, d:curvePath(e.s,e.t) }));
  return (
    <div className="w-full rounded-2xl overflow-hidden border border-white/10 bg-[#0a1220]">
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-white/[.07] bg-[#111c2d]">
        {["#ef4444","#f59e0b","#22c55e"].map(c=><div key={c} className="w-3 h-3 rounded-full opacity-70" style={{background:c}}/>)}
        <span className="mono text-white/30 text-[11px] ml-2">OpenGate IAM — Service Architecture</span>
        <div className="ml-auto flex gap-1.5">
          {[["gateway","#00B4D8"],["core","#10b981"],["secondary","#8b5cf6"],["infra","#f59e0b"],["secrets","#FFCA28"]].map(([g,c])=>(
        ...
        <svg viewBox="0 0 900 500" className="w-full min-w-[600px] block">
          ...
          {edges.map(e=>(
            <g key={e.id}>
              ...
              <path id={e.id} d={e.d} fill="none" stroke={e.color} strokeWidth={e.anim?1.5:1}
          ...
          {NODES.map(n=>{
            const ih=hov===n.id, rx=n.cx-n.w/2, ry=n.cy-n.h/2;
            return(
              <g key={n.id} className="cursor-pointer" onMouseEnter={()=>setHov(n.id)} onMouseLeave={()=>setHov(null)}>
                ...
                <circle cx={rx+n.w-10} cy={n.cy} r={ih?4.5:3} fill={n.color} opacity={ih?1:.55} className="anim-npulse"/>
                <text x={n.cx-5} y={n.cy-(n.sub?7:0)} textAnchor="middle" dominantBaseline="middle"
                  fontSize={n.id==="gateway"?11.5:n.id==="client"?11:10} fontWeight={600}
                  fill={ih?n.color:"rgba(255,255,255,.82)"} fontFamily="monospace"
                  style={{transition:"fill .2s"}}>{n.label}</text>
                {n.sub&&<text x={n.cx-5} y={n.cy+9} textAnchor="middle" dominantBaseline="middle"
                  fontSize={8} fill={ih?`${n.color}cc`:"rgba(255,255,255,.3)"} fontFamily="monospace">{n.sub}</text>}
              </g>
            );
          })}
          {[["CLIENTS",10,22,"#64748b"],["GATEWAY",10,98,"#00B4D8"],["CORE SERVICES",10,196,"#10b981"],["SECONDARY",10,308,"#8b5cf6"],["INFRASTRUCTURE",10,422,"#f59e0b"]].map(([l,x,y,c])=>(
...
function useSectionVis() {
  const refs = useRef({});
  const [vis, setVis] = useState(new Set());
  useEffect(() => {
    const obs = new IntersectionObserver(
      entries => entries.forEach(e => { if(e.isIntersecting) setVis(p=>new Set([...p,e.target.dataset.sec])); }),
      { threshold:0.1 }
    );
    Object.values(refs.current).forEach(el=>el&&obs.observe(el));
    return()=>obs.disconnect();
  }, []);
  const ref = id => el => { refs.current[id]=el; if(el) el.dataset.sec=id; };
  const cls = id => `sec-enter ${vis.has(id)?"sec-show":""}`;
  return { ref, cls };
}
  ...
  return (
    <div className="min-h-screen bg-white" style={{fontFamily:"'Outfit',sans-serif"}}>
      ...
      <nav className={`fixed top-0 inset-x-0 z-50 border-b border-white/10 backdrop-blur-xl transition-all duration-300 ${scrolled?"shadow-[0_4px_24px_rgba(0,0,0,.4)] bg-[#0D1B2A]/97":"bg-[#0D1B2A]/90"}`}>
        <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-[30px] h-[30px] rounded-lg flex items-center justify-center" style={{background:"linear-gradient(135deg,#00B4D8,#0077a8)",boxShadow:"0 0 12px rgba(0,180,216,.4)"}}>
              <ShieldSVG/>
            </div>
            <span className="font-bold text-white text-sm">OpenGate IAM</span>
            <span className="mono text-[10px] px-1.5 py-0.5 rounded-full ml-1" style={{color:CYAN,border:`1px solid ${CYAN}40`,background:`${CYAN}12`}}>{VERSION}</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            <a href="https://github.com/MuyleangIng/opengate-iam" target="_blank" rel="noopener noreferrer" className="text-white/45 hover:text-white transition-colors"><GithubSVG size={18}/></a>
            <Link href="/docs/getting-started" className="text-sm font-semibold text-white px-4 py-1.5 rounded-lg hover:opacity-85 transition-opacity" style={{background:"linear-gradient(135deg,#00B4D8,#0077a8)",boxShadow:"0 2px 12px rgba(0,180,216,.3)"}}>Get Started</Link>
          </div>
        </div>
      </nav>
      ...
      <section className="pt-14 relative overflow-hidden mesh-bg">
        <div className="absolute inset-0 opacity-[.035]" style={{backgroundImage:"linear-gradient(rgba(255,255,255,.4) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.4) 1px,transparent 1px)",backgroundSize:"44px 44px"}}/>
        <div className="absolute top-[10%] left-[12%] w-96 h-96 rounded-full pointer-events-none" style={{background:"radial-gradient(circle,rgba(0,180,216,.13) 0%,transparent 70%)"}}/>
        <div className="absolute top-[20%] right-[8%] w-72 h-72 rounded-full pointer-events-none" style={{background:"radial-gradient(circle,rgba(139,92,246,.1) 0%,transparent 70%)"}}/>

        <div className="relative max-w-4xl mx-auto px-6 py-20 text-center">
          {/* badge */}
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border mb-7 anim-shimmer hero-grad anim-fadeup" style={{borderColor:`${CYAN}40`,color:CYAN,fontSize:11}}>
          ...
          <div className="flex justify-center mb-7 anim-fadeup">
            <div className="w-20 h-20 rounded-[28px] flex items-center justify-center anim-float" style={{background:"linear-gradient(135deg,#00B4D8,#0077a8)",boxShadow:"0 0 40px rgba(0,180,216,.35)"}}>
              <ShieldSVG size={38}/>
            </div>
          </div>
          ...
          <h1 className="text-5xl md:text-7xl font-extrabold text-white leading-[1.1] tracking-tight mb-5 anim-fadeup">
            Open-Source<br/>
            <span className="anim-bgmove" style={{background:"linear-gradient(90deg,#00B4D8,#7dd3fc,#00B4D8)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Identity & Access</span>
            <br/>Management
          </h1>
      ...
      <section className="bg-white border-b border-slate-100 py-7">
        <div className="max-w-2xl mx-auto px-6 grid grid-cols-4 gap-6 text-center">
          {[["11","Microservices"],["OAuth 2.1","Standard"],["MIT","License"],["Java 21","Runtime"]].map(([v,l])=>(
            <div key={l}><p className="text-2xl font-extrabold text-slate-900">{v}</p><p className="text-[11px] text-slate-400 mt-0.5">{l}</p></div>
          ))}
        </div>
      </section>
      ...
      <section ref={ref("gs")} className={`py-20 bg-white ${cls("gs")}`}>
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
            {gettingStarted.map(({icon,title,href,desc,color})=>(
              <Link key={title} href={href} className="group flex items-start gap-3.5 p-4 rounded-2xl border border-slate-200 bg-white hover:-translate-y-1 hover:shadow-[0_12px_28px_rgba(0,180,216,.1)] transition-all duration-200">
      ...
      <section ref={ref("arch")} className="py-20 bg-slate-50 border-y border-slate-200">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="flex flex-wrap gap-3 justify-center mt-5">
            {[["API Gateway","#00B4D8"],["Core Services","#10b981"],["Secondary","#8b5cf6"],["Infrastructure","#f59e0b"],["Vault / Secrets","#FFCA28"],["● Animated flow","#64748b"]].map(([l,c])=>(
      ...
      <section className="py-20 relative overflow-hidden mesh-bg">
        <div className="absolute inset-0 opacity-[.03]" style={{backgroundImage:"linear-gradient(rgba(255,255,255,.5) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.5) 1px,transparent 1px)",backgroundSize:"40px 40px"}}/>
        <div className="relative max-w-2xl mx-auto px-6 text-center">
          ...
          <div className="rounded-2xl overflow-hidden border border-white/10 text-left mb-7">
            <div className="flex items-center gap-2 px-4 py-2.5 border-b border-white/[.07] bg-white/[.05]">
              {["#ef4444","#f59e0b","#22c55e"].map(c=><div key={c} className="w-3 h-3 rounded-full opacity-65" style={{background:c}}/>)}
...
# Auth Endpoint  →  http://localhost:9080`}</pre>
          ...
          <div className="flex justify-center gap-3 flex-wrap">
            <Link href="/docs/getting-started/quick-start" className="inline-flex items-center gap-2 text-sm font-semibold px-6 py-3 rounded-xl text-white hover:opacity-85 transition-opacity" style={{background:CYAN}}>
              Full Quick Start <ArrowSVG size={13}/>
            </Link>
            <Link href="/docs/deployment" className="text-sm font-medium px-6 py-3 rounded-xl border border-white/20 text-white/65 hover:bg-white/5 transition-colors">
              Docker Compose Docs
            </Link>
          </div>
        </div>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/80">
                  {["Service","Port","Responsibility"].map(h=>(
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            {[
              ...
              <a key={c.label} href={c.href} target="_blank" rel="noopener noreferrer"
                ...
                <div className="w-[52px] h-[52px] rounded-2xl flex items-center justify-center text-2xl mb-4 group-hover:scale-110 transition-transform duration-200" style={{background:`${c.color}12`}}>{c.icon}</div>
                <p className="font-bold text-slate-900 mb-1.5">{c.label}</p>
                <p className="text-[12px] text-slate-400 leading-relaxed">{c.sub}</p>
                <span className="mt-4 text-[12px] font-semibold" style={{color:c.color}}>{c.cta}</span>
              </a>
            ))}
          </div>
          <div className="rounded-2xl border border-rose-100 p-7 flex flex-wrap items-center justify-between gap-5" style={{background:"linear-gradient(135deg,#fff1f2,#fdf4ff)"}}>
            <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-10 mb-12">
            <div className="col-span-2">
              <div className="flex items-center gap-2.5 mb-4">
                ...
                <span className="mono text-[10px] px-1.5 py-0.5 rounded-full" style={{color:CYAN,border:`1px solid ${CYAN}30`,background:`${CYAN}12`}}>{VERSION}</span>
              </div>
            ...
            {[
              ...
              <div key={col.title}>
                <p className="text-[10px] text-white/35 font-bold uppercase tracking-[.18em] mb-4">{col.title}</p>
                <ul className="space-y-2.5 list-none">
                  {col.links.map(([l,h])=>(
                    <li key={h}><Link href={h} className="text-[12px] text-white/35 hover:text-white/80 transition-colors">{l}</Link></li>
                  ))}
                </ul>
              </div>
            ))}
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-extrabold text-sm flex-shrink-0" style={{background:"linear-gradient(135deg,#00B4D8,#8b5cf6)"}}>M</div>
                <div>
                  <p className="text-[13px] font-bold text-white/80">Ing Muyleang</p>
                  <p className="text-[11px] text-white/28">Full-Stack Engineer</p>
                </div>
              </div>
              {[["muyleanging.com","https://muyleanging.com"],["khmerstack.muyleanging.com","https://khmerstack.muyleanging.com"]].map(([l,h])=>(
                <a key={h} href={h} target="_blank" rel="noopener noreferrer" className="block text-[11px] mb-2 opacity-60 hover:opacity-100 transition-opacity" style={{color:CYAN}}>↗ {l}</a>
              ))}
            </div>
          </div>
